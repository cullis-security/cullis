...
if docker compose version &>/dev/null 2>&1; then
    ...
    # ./certs, build: .) resolving against the repo root even though the
    # compose files themselves now live under deploy/compose/. The base
    # compose file is baked in so callers without an explicit -f keep
    # working (down/rebuild/logs).
    COMPOSE="docker compose --project-directory $SCRIPT_DIR -f deploy/compose/docker-compose.yml"
...
NGINXEOF
    ...
    mkdir -p "$SCRIPT_DIR/deploy/compose"
    cat > "$SCRIPT_DIR/deploy/compose/docker-compose.letsencrypt.yml" <<COMPOSEEOF
...
if [[ "$MODE" == "production" ]]; then
    ...
    if [[ "$USE_LETSENCRYPT" == "y" ]]; then
        COMPOSE_FILES="$COMPOSE_FILES -f deploy/compose/docker-compose.letsencrypt.yml"
    fi
