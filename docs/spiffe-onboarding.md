...
> **Deprecation notice (ADR-011 / ADR-014).** `CullisClient.from_spiffe_workload_api()` as shown in Step 4 below is the **legacy direct-to-Court login** path. It still works and emits a `DeprecationWarning` until the Court's `/v1/auth/token` returns `410 Gone` (see the `Deprecation` / `Sunset` response headers). New deployments should use SPIFFE as an *enrollment* primitive instead — call `CullisClient.enroll_via_spiffe(...)` once at provisioning time to exchange an SVID for an Org-CA-signed cert+key pair pinned on the Mastio, then run the agent with `CullisClient.from_identity_dir(cert_path=, key_path=, dpop_key_path=)` at runtime (the cert presented at the per-org nginx sidecar's TLS handshake IS the agent credential — ADR-014 PR-C dropped api_key entirely). See [`migration-from-direct-login.md`](migration-from-direct-login.md) for the full migration walkthrough.
